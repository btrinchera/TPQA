...
int mlink_cmd(TMLink *lnk,char *cmd,char *ret,int retmax)
{
	if(lnk->mode == ML_MATLAB)
	{
		...
		// wrap command to try-catch
		char *buf = (char*)malloc((strlen(cmd) + 256)*sizeof(char));
		strcpy(buf, "twm_is_err = '';\n");
		strcat(buf, "try ");
		strcat(buf, cmd);
		strcat(buf, ", catch twm_is_err = 'error'; rethrow(lasterror);, end\n");

		// eval command
		engEvalString(lnk->eng,buf);
		...
		if(ret)
		{
			...
			char *errmsg = NULL;
			int err = mlink_get_var_str(lnk, "twm_is_err",&errmsg);
			if(!err && strlen(errmsg))
			{
				// yes
				return(1);
			}
